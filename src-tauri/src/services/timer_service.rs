...
pub fn start(conn: &Connection, task_id: &str) -> Result<(), AppError> {
    let proj_id = BusinessRepository::get_project_id_by_task_id(conn, task_id)?;

    let now = Utc::now().to_rfc3339();
    BusinessRepository::close_active_logs_by_project(conn, &now, &proj_id)?;
    ...
    BusinessRepository::insert_time_log(conn, &log_id, task_id, &now)?;
    Ok(())
}
...
pub fn stop(conn: &Connection, project_id: Option<&str>) -> Result<(), AppError> {
    ...
    match project_id {
        Some(p_id) => {
            BusinessRepository::close_active_logs_by_project(conn, &now, p_id)?;
        }
        None => {
            BusinessRepository::close_all_active_logs(conn, &now)?;
        }
    }
    Ok(())
}
...
pub fn get_active(conn: &Connection) -> Result<Vec<String>, AppError> {
    let ids = BusinessRepository::query_active_logs(conn)?;
    Ok(ids)
}
