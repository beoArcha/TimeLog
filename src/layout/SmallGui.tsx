    ...
    >
      <div className={`rounded-2xl border shadow-2xl p-3 flex flex-col gap-2.5 relative overflow-hidden transition-all duration-300 w-full h-full ${resolvedTheme === 'light'
            ...
            <label className="flex items-center gap-0.5 font-mono text-[10px] mr-1 text-slate-400 hover:text-orange-400 cursor-pointer select-none">
              <input
                ...
                onChange={(e) => {
                  ...
                  showToast(e.target.checked ? translate(locale, SmallGuiKey.OnTopOn, customTranslations) : translate(locale, SmallGuiKey.OnTopOff, customTranslations));
                }}
          ...
          {activeProj && (
            <div className="flex items-center gap-2 truncate">
              ...
              <div className="text-left">
                <p className="text-[10px] font-mono text-slate-400 uppercase tracking-widest leading-none">{translate(locale, SmallGuiKey.AppProfile, customTranslations)}</p>
        ...
        <div className="pt-1 text-center">
          ...
          >
            <span>{isSmallExpanded ? translate(locale, SmallGuiKey.HideTasks, customTranslations) : translate(locale, SmallGuiKey.ShowTasks, customTranslations)}</span>
        ...
        <AnimatePresence>
          {isSmallExpanded && (
            ...
            >
              {(() => {
                if (!activeProj) return <p className="text-xs italic text-slate-400 text-center py-4">{translate(locale, DynamicKey.NoProjects, customTranslations)}</p>;

                const projRootTasks = tasks.filter(t => t.projectId === activeProj.id && t.parentTaskId === null);

                if (projRootTasks.length === 0) {
                  return <p className="text-xs italic text-slate-400 text-center py-4">{translate(locale, DynamicKey.NoTasksInProfile, customTranslations)}</p>;
                }
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              onClick={() => {
                                if (isRootActive) {
                                  handleStopTimer();
                                  showToast(`${translate(locale, SmallGuiKey.StoppedMeasurement, customTranslations)}${task.name}`);
                                } else {
                                  handleStartTimer(task.id);
                                  showToast(`${translate(locale, SmallGuiKey.StartedMeasurement, customTranslations)}${task.name}`);
                                }
                              }}
                              ...
                              title={isRootActive ? translate(locale, SmallGuiKey.StopMeasurement, customTranslations) : translate(locale, SmallGuiKey.StartMeasurement, customTranslations)}
                            >
                                  ...
                                  <div key={sub.id} className="flex items-center justify-between gap-2 py-0.5">
                                    ...
                                    <button
                                      onClick={() => {
                                        if (isSubActive) {
                                          handleStopTimer();
                                          showToast(`${translate(locale, SmallGuiKey.StoppedSubtask, customTranslations)}${sub.name}`);
                                        } else {
                                          handleStartTimer(sub.id);
                                          showToast(`${translate(locale, SmallGuiKey.StartedSubtask, customTranslations)}${sub.name}`);
                                        }
                                      }}
