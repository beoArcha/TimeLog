    ...
    <div className={`p-6 rounded-[2rem] border shadow-2xl flex flex-col gap-6 transition-all duration-300 ${theme === 'light' ? 'bg-[#FCFAF8] border-[#DFD7CB] shadow-[#DFD7CB]' : 'bg-[#FCFAF8]/5 border-white/10'
      ...
      <div>
        <span className="text-[10px] font-mono tracking-wider bg-orange-500/20 text-orange-400 px-3 py-1 rounded-full font-bold uppercase border border-orange-500/25">
          {translate(locale, 'calendar', 'ResourceDatabaseTable', customTranslations)}
        </span>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mt-1.5">
          <h2 className={`font-sans font-bold text-2xl flex items-center gap-2 ${theme === 'light' ? 'text-[#2C2421]' : 'text-white'
            ...
            {translate(locale, 'calendar', 'HolidaysVacations', customTranslations)}
          </h2>
        </div>
      </div>

      <CollapsibleCard
        title={translate(locale, 'calendar', 'RegisterNewDayOff', customTranslations)}
        icon={Plus}
        ...
        <form onSubmit={handleAddHolidaySubmit} className="mt-6 flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono tracking-wider text-[#9B8C83] uppercase">{translate(locale, 'common', 'Date', customTranslations)}</label>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono tracking-wider text-[#9B8C83] uppercase">{translate(locale, 'common', 'Type', customTranslations)}</label>
              <select value={newHolidayType} onChange={e => setNewHolidayType(e.target.value as any)} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-mono ${theme === 'light' ? 'bg-[#FCFAF8] border-[#DFD7CB] text-[#2C2421]' : 'bg-black/50 border-white/10 text-white'}`}>
                <option value="leave">{translate(locale, 'calendar', 'VacationLeave', customTranslations)}</option>
                <option value="holiday">{translate(locale, 'calendar', 'OfficialHoliday', customTranslations)}</option>
              </select>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono tracking-wider text-[#9B8C83] uppercase">{translate(locale, 'calendar', 'DescriptionCause', customTranslations)}</label>
              <input type="text" value={newHolidayName} onChange={e => setNewHolidayName(e.target.value)} placeholder={translate(locale, 'calendar', 'EGVibeDancingLeave', customTranslations)} required className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-sans ${theme === 'light' ? 'bg-[#FCFAF8] border-[#DFD7CB] text-[#2C2421] placeholder:text-[#9B8C83]' : 'bg-black/50 border-white/10 text-white placeholder:text-[#8A7A71]'}`} />
            </div>
          </div>
          <button type="submit" className="mt-4 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-orange-500/20 transition-all w-full md:w-auto">
            <Plus className="w-4 h-4" /> {translate(locale, 'common', 'Save', customTranslations)}
          </button>
        </form>
      </CollapsibleCard>

      <CollapsibleCard
        title={`📋 ${translate(locale, 'calendar', 'SqliteHolidaysTableRows', customTranslations)} (${holidays.length})`}
        icon={Database}
          ...
          <table className="w-full text-sm font-sans text-left whitespace-nowrap">
            ...
            <thead>
              <tr className={`border-b text-xs font-bold font-mono tracking-wider uppercase ${theme === 'light' ? 'border-[#DFD7CB] text-[#7A6A61] bg-[#F4EFEA]/50' : 'border-white/10 text-[#9B8C83] bg-black/40'}`}>
                <th className="py-2.5 px-4">{translate(locale, 'calendar', 'RecordName', customTranslations)}</th>
                <th className="py-2.5 px-4">{translate(locale, 'calendar', 'ScheduledDate', customTranslations)}</th>
                <th className="py-2.5 px-4">{translate(locale, 'calendar', 'Weekday', customTranslations)}</th>
                <th className="py-2.5 px-4">{translate(locale, 'common', 'Type', customTranslations)}</th>
                <th className="py-2.5 px-4 text-right">{translate(locale, 'common', 'Actions', customTranslations)}</th>
              </tr>
            </thead>
            <tbody>
              {holidays.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-12 text-center text-[#9B8C83] text-xs font-mono">
                    {translate(locale, 'calendar', 'NoEntriesFound', customTranslations)}
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={h.id} className={`border-b last:border-b-0 ${theme === 'light' ? 'border-[#EAE4DB] hover:bg-[#F4EFEA] text-[#5A4A42]' : 'border-white/5 hover:bg-[#FCFAF8]/5 text-slate-300'}`}>
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider ${h.type === 'holiday'
                          ...
                          {h.type === 'holiday' ? translate(locale, 'calendar', 'Holiday', customTranslations) : translate(locale, 'calendar', 'Vacation', customTranslations)}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right">
                        <button onClick={() => handleDeleteHoliday(h.id)} className="p-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 transition-colors" title={translate(locale, 'calendar', 'DeleteSqlDelete', customTranslations)}>
