    ...
    <div>
      <div className={`flex flex-col ${isCondensed ? 'gap-2 items-start' : 'sm:flex-row sm:items-start justify-between gap-2'} border-b pb-5 mb-5 ${theme === 'light' ? 'border-[#DFD7CB]' : 'border-white/10'
        ...
        <div className={`flex-1 min-w-0 ${isCondensed ? 'w-full' : ''}`}>
          <span className={`text-main tracking-wider bg-orange-500/20 text-orange-500 dark:text-orange-300 px-3 py-1 rounded-full font-bold uppercase border border-orange-500/25 ${isCondensed ? 'inline-block mb-2 whitespace-nowrap' : ''}`}>
            {translate(locale, 'project', 'SelectProject', customTranslations)}
          </span>
          <h2 className={`font-sans font-bold mt-1.5 flex items-center gap-main text-giant ${theme === 'light' ? 'text-[#2C2421]' : 'text-white'
            }`}>
            <span className={`icon-medium rounded-full bg-[var(--project-color,orange)] shrink-0 shadow-md`} style={{ backgroundColor: selectedProject.color ? `var(--tw-color-${selectedProject.color}-500)` : undefined }} />
            <span className="truncate" title={selectedProject.name}>{selectedProject.name}</span>
          </h2>

          {isCondensed && (
            <div className={`mt-3 font-mono text-main border px-3 py-1.5 rounded-2xl flex flex-wrap items-center gap-2 w-fit ${theme === 'light' ? 'bg-[#F4EFEA] border-[#DFD7CB] text-[#5A4A42]' : 'bg-[#FCFAF8]/5 border-white/10 text-slate-300'
              }`}>
        ...
        {!isCondensed && (
          <div className={`text-left sm:text-right font-mono text-title border padding-section rounded-section flex flex-wrap items-center gap-2 transition-all w-fit ${theme === 'light'
      ...
      <form onSubmit={onAddTaskSubmit} className={`flex gap-3 ${isCondensed ? 'mt-2' : ''}`}>
        <input
          ...
          className={`flex-1 px-4 input-py border rounded-main text-main focus:outline-none focus:ring-2 focus:ring-orange-400 transition-all ${theme === 'light'
        ...
        <button
          id="add-task-btn"
          type="submit"
          className={`bg-gradient-to-tr from-orange-400 to-rose-500 hover:from-orange-500 hover:to-rose-600 text-white font-semibold rounded-main px-5 sm:px-6 py-3 sm:py-3.5 text-main flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md shrink-0`}
        >
          <Plus className="icon-medium" /> <span className={isCondensed ? 'hidden xs:inline' : ''}>{translate(locale, 'task', 'AddTask', customTranslations)}</span>
        </button>
      </form>
    </div>
  );
