...
export function TaskVisibilityToggle({
  ...
  const label = isExpanded
    ? translate(locale, SmallGuiKey.HideTasks, customTranslations)
    : translate(locale, SmallGuiKey.ShowTasks, customTranslations);

  return (
    <div className="pt-1 text-center">
      <button
        onClick={onToggle}
        className={`w-full py-1 rounded-xl transition-all font-mono text-[9px] uppercase font-bold tracking-wider flex items-center justify-center gap-1.5 cursor-pointer ${
          resolvedTheme === 'light'
      ...
      >
        ...
        <div className="w-4 h-4 rounded-full bg-orange-500/10 flex items-center justify-center">
          <ChevronDown
            className={`w-3 h-3 transition-transform duration-300 text-orange-400 ${
