    ...
    <AnimatePresence mode="wait">
      {activeLog ? (
        <motion.div
          ...
          className={`padding-main rounded-main border shadow-2xl flex flex-col ${isCondensed ? 'items-start' : 'sm:flex-row items-center justify-between'} gap-main relative overflow-hidden backdrop-blur-md transition-all ${theme === 'light'
        ...
        >
          ...
          <div className={`flex items-center gap-main z-10 ${isCondensed ? 'w-full' : 'sm:w-auto w-full'}`}>
            <div className="relative">
              <div className={`icon-large shrink-0 rounded-2xl flex items-center justify-center border shadow-lg animate-spin-slow ${theme === 'light' ? 'bg-[#FCFAF8] border-[#DFD7CB] text-orange-500' : 'bg-[#FCFAF8]/10 border-white/10 text-orange-450'
                }`}>
            ...
            <div className="min-w-0">
              ...
              <h4 className={`font-sans font-extrabold text-giant mt-1 leading-snug truncate ${theme === 'light' ? 'text-[#2C2421]' : 'text-white'
          ...
          <div className={`flex items-center gap-main z-10 w-full ${isCondensed ? 'justify-between pt-4 border-t' : 'sm:w-auto justify-between sm:justify-end border-t sm:border-0 pt-3 sm:pt-0'} ${theme === 'light' ? 'border-[#DFD7CB]' : 'border-white/10'
            }`}>
            ...
            <button
              id="stop-timer-btn"
              onClick={() => onStopTimer()}
              className={`bg-rose-500 hover:bg-rose-600 text-white rounded-main padding-section transition-all duration-300 flex items-center justify-center shadow-lg transform active:scale-95 group cursor-pointer`}
            >
              <Square className={`icon-medium text-white fill-white group-hover:scale-110 transition-transform`} />
            </button>
          </div>
        </motion.div>
      ) : (
        <div id="active-timer-idle-banner" className={`backdrop-blur-md rounded-main padding-main border flex flex-col ${isCondensed ? 'gap-main' : `md:flex-row items-center justify-between gap-main`} transition-all duration-300 ${theme === 'light'
          ? 'bg-[#FCFAF8] border-[#DFD7CB] shadow-sm shadow-[#DFD7CB]/50'
          : 'bg-[#FCFAF8]/5 border-white/10'
          }`}>
          <div className={`flex items-center gap-main`}>
            <div className={`icon-large border rounded-section flex items-center justify-center shrink-0 ${theme === 'light' ? 'bg-[#F4EFEA] border-[#DFD7CB]' : 'bg-[#FCFAF8]/5 border-white/10'
              }`}>
