...
export default function BaseGui({ state, isCondensed }: { state: GuiState; isCondensed: boolean }) {
  const { projects, tasks, logs, activeLog, onAddProject, onAddTask, onRenameProject, onRenameTask, onToggleTaskComplete, onDeleteTask, onStartTimer, onStopTimer, onToggleProjectArchive, nowIso, locale, customTranslations, theme, holidays, setHolidays, patches, sysSettings, selectedTaskId, setSelectedTaskId, activeView = "tasks", selectedProjectId, setSelectedProjectId, newHolidayDate, setNewHolidayDate, newHolidayType, setNewHolidayType, newHolidayName, setNewHolidayName, reportPeriod, setReportPeriod, reportSort, setReportSort, newProjectName, setNewProjectName, newProjectColor, setNewProjectColor, newTaskName, setNewTaskName, selectedParentTaskId, setSelectedParentTaskId, newSubtaskName, setNewSubtaskName, showSubtaskFormForId, setShowSubtaskFormForId, editingId, setEditingId, editName, setEditName, showDbInspector, setShowDbInspector, selectedProject, projectTasks, rootTasks } = state;
  ...
  return (
    <div id="gui-container" className={`grid grid-cols-1 ${isCondensed ? '' : 'lg:grid-cols-12'} gap-8 ${
                      ...
                      <div>
                        {editingId === p.id ? (
                          <input
                            ...
                            className={`font-semibold text-sm rounded px-1 outline-none ${
                              theme === 'light' ? 'bg-white text-[#2C2421] border-[#DFD7CB]' : 'bg-black text-white border-white/20'
                            } border`}
                          />
                        ) : (
      ...
      <div id="tasks-main" className={`${isCondensed ? 'lg:col-span-12' : 'lg:col-span-8'} flex flex-col gap-6`}>
              ...
              <div className={`flex items-center gap-4 z-10 ${isCondensed ? 'w-full' : 'sm:w-auto w-full'}`}>
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {activeProject && (
                      <span className={`text-xs font-semibold drop-shadow-sm flex items-center gap-1.5 min-w-0 ${
        ...
        {activeView === 'tasks' && (
          ...
          <div id="project-tasks-sheet" className={`backdrop-blur-md rounded-[2.5rem] p-8 border shadow-2xl flex flex-col gap-6 transition-all duration-300 ${
            theme === 'light'
              ...
              <div>
                <div className={`flex flex-col ${isCondensed ? 'gap-2 items-start' : 'sm:flex-row sm:items-start justify-between gap-2'} border-b pb-5 mb-5 ${
                  theme === 'light' ? 'border-[#DFD7CB]' : 'border-white/10'
                }`}>
                  <div className={`flex-1 min-w-0 ${isCondensed ? 'w-full' : ''}`}>
                    ...
                    <h2 className={`font-sans font-bold mt-1.5 flex items-center gap-2 ${isCondensed ? 'text-xl' : 'text-2xl'} ${
                      theme === 'light' ? 'text-[#2C2421]' : 'text-white'
                    }`}>
                    ...
                    {isCondensed && (
                      <div className={`mt-3 font-mono text-xs border px-3 py-1.5 rounded-2xl flex flex-wrap items-center gap-2 w-fit ${
                        theme === 'light' ? 'bg-[#F4EFEA] border-[#DFD7CB] text-[#5A4A42]' : 'bg-[#FCFAF8]/5 border-white/10 text-slate-300'
                      }`}>
                        <div className="flex items-center gap-2">
                  ...
                  {!isCondensed && (
                    <div className={`text-left sm:text-right font-mono text-sm border px-3 py-1.5 rounded-2xl flex flex-wrap items-center gap-2 transition-all w-fit ${
                      theme === 'light'
                        ? 'bg-[#F4EFEA] border-[#DFD7CB] text-[#5A4A42]'
                        : theme === 'high-contrast'
                        ? 'bg-black border-white text-white'
                ...
                <form onSubmit={handleAddTaskSubmit} className={`flex gap-3 ${isCondensed ? 'mt-2' : ''}`}>
                  <input
                    ...
                    className={`flex-1 px-4 py-3 sm:py-3.5 border rounded-[1.25rem] text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 transition-all ${
                      theme === 'light'
                ...
                rootTasks.map(rootTask => {
                  ...
                  return (
                    <div 
                      ...
                      key={rootTask.id} 
                      className={`rounded-3xl p-5 border transition-all flex flex-col gap-4 group/root relative overflow-hidden backdrop-blur-md ${
                        isAnyRunning 
                          ? theme === 'light'
                    ...
                    >
                      ...
                      <div className={`flex ${isCondensed ? 'flex-col gap-3.5' : 'items-center justify-between gap-4'} animate-fade-in pl-1`}>
                          ...
                          <div className="min-w-0 flex-1 flex items-center justify-between group/taskedit">
                            {editingId === rootTask.id ? (
                              <input
                                ...
                                className={`font-semibold text-sm rounded px-1 outline-none w-full max-w-sm mr-2 ${
                                  theme === 'light' ? 'bg-white text-[#2C2421] border-[#DFD7CB]' : 'bg-black text-white border-white/20'
                                } border`}
                              />
                            ) : (
                              <span className={`font-semibold text-sm flex flex-wrap items-center gap-2 transition-all duration-300 min-w-0 w-full ${
                                rootTask.completed
                        ...
                        <div className={`flex items-center gap-3 ${isCondensed ? 'w-full justify-between pt-3 border-t ' + (theme === 'light' ? 'border-[#DFD7CB]' : 'border-white/10') : ''}`}>
                          <span className={`text-xs font-mono font-extrabold px-3 py-1.5 rounded-full border transition-all ${
                            isAnyRunning 
                      ...
                      {/* Subtasks Rendering */}
                      {subTasks.length > 0 && (
                        <div className={`flex flex-col gap-2 ml-8 border-l pl-4 mt-2 overflow-y-auto pr-1 ${isCondensed ? 'max-h-[250px]' : ''} ${
                          theme === 'light' ? 'border-[#DFD7CB]' : 'border-white/10'
                        }`}>
                            ...
                            return (
                              ...
                              >
                                  ...
                                  <div className="min-w-0 flex-1 flex items-center justify-between group/taskedit">
                                    {editingId === subTask.id ? (
                                      <input
                                        ...
                                        className={`font-semibold text-xs rounded px-1 outline-none w-full max-w-sm mr-2 ${
                                          theme === 'light' ? 'bg-white text-[#2C2421] border-[#DFD7CB]' : 'bg-black text-white border-white/20'
                                        } border`}
                                      />
                                    ) : (
                                      <span className={`text-xs font-semibold flex flex-wrap items-center gap-2 transition-all duration-300 min-w-0 w-full ${
                                        subTask.completed
                                          ? 'line-through text-[#9B8C83] font-normal'
                                          : theme === 'light'
                                          ? 'text-[#2C2421]'
                                ...
                                <div className={`flex items-center gap-3 shrink-0`}>
                                  <span className={`font-mono text-[10px] px-2 py-1 rounded-md border transition-all ${
                                    isSubRunning 
                                  ...
                                  {isSubRunning ? (
                                    ...
                                    <button
                                      ...
                                      className={`text-[#9B8C83] hover:text-white rounded-lg p-2 transition-all cursor-pointer shrink-0 ${
                                        theme === 'light' ? 'bg-[#EAE4DB] hover:bg-teal-500 group-hover/sub:bg-teal-500 text-[#5A4A42]' : 'bg-[#FCFAF8]/5 hover:bg-teal-500 group-hover/sub:bg-teal-500'
                                      }`}
                                    >
        ...
        {/* DB Schema Inspector Expandable Card */}
        <div className={`mt-2 flex flex-col rounded-3xl border transition-all duration-300 overflow-hidden ${
          theme === 'light'
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-xl flex items-center justify-center ${
                theme === 'light' ? 'bg-[#F4EFEA] border border-[#DFD7CB]' : 'bg-[#FCFAF8]/10 border border-white/10'
              }`}>
          ...
          <AnimatePresence>
            {showDbInspector && (
              ...
              >
                <div className={`p-4 pt-4 md:p-6 md:pt-4 border-t font-mono text-xs gap-4 flex flex-col ${
                  theme === 'light' ? 'border-[#DFD7CB]/50' : 'border-white/10'
                }`}>
                  <div className={`flex flex-col gap-4 pb-2 ${!isCondensed ? 'md:grid md:grid-cols-2' : ''}`}>
                    {/* Projects Table */}
                    <div className={`flex-1 p-3 rounded-xl border flex flex-col ${
                    ...
                    <div className={`flex-1 w-full p-3 rounded-xl border flex flex-col ${!isCondensed ? 'md:col-span-2' : ''} ${
                      theme === 'light' ? 'bg-[#F4EFEA]/50 border-[#DFD7CB]' : 'bg-black/20 border-white/10'
                    }`}>
