  ...
  const executeCommand = (cmdText: string) => {
    ...
    switch (commandName) {
      ...
      case 'oxyhelp': {
        ...
        outputs.push(
          { text: isPl ? '================ Dostępne Polecenia OxyFlowCLI ================' : '================ Available OxyFlowCLI Commands ================', type: 'info' },
          { text: `  projects                      - ${isPl ? 'Listuje projekty w bazie sqlite' : 'Lists all projects in the SQLite database'}`, type: 'output' },
          { text: `  tasks <id_projektu/proj_id>   - ${isPl ? 'Listuje zadania i podzadania danego projektu' : 'Lists all tasks and subtasks of a project'}`, type: 'output' },
          { text: `  addproject "<nazwa/name>"     - ${isPl ? 'Tworzy nowy projekt (nazwa w cudzysłowie)' : 'Creates a new project (name in quotes)'}`, type: 'output' },
          { text: `  addtask <pId> "<nazwa/name>"  - ${isPl ? 'Tworzy zadanie główne w projekcie' : 'Creates a main task in the project'}`, type: 'output' },
          { text: `  addsubtask <parent_id> "<n>"  - ${isPl ? 'Tworzy podzadanie dla zadania nadrzędnego' : 'Creates a subtask for a parent task'}`, type: 'output' },
          { text: `  start [task_id]               - ${isPl ? 'Rozpoczyna mierzenie czasu dla zadania (bez id: obecnie ustawiony task)' : 'Starts timer for task_id (no id: starts currently selected task)'}`, type: 'output' },
          { text: `  stop [all]                    - ${isPl ? 'Zatrzymuje aktywny timer (argument: "all" - zatrzymuje wszystko)' : 'Stops the active timer (argument: "all" - stops everything)'}`, type: 'output' },
          ...
          { text: `  time <subtask/task/profile> <id> [today/week/month] - Zliczenie czasu pracy dla zadania/podzadania/profilu w danym okresie`, type: 'output' },
