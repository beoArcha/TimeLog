...
import { AppWindow, Layers, Cpu } from 'lucide-react';
...
export default function TrayWidget({ onRestore, onStopAll, showToast }: TrayWidgetProps) {
  const { 
    projects, 
    tasks, 
    logs, 
  ...
  return (
    ...
    >
      <div className={`rounded-[2.5rem] p-8 shadow-3xl w-full text-center flex flex-col items-center gap-6 relative overflow-hidden group border transition-all duration-300 ${
        resolvedTheme === 'light'
        ...
        <div className="relative">
          ...
          <span className="absolute -top-1 -right-1 flex h-5 w-5">
            ...
            <span className={`relative inline-flex rounded-full h-5 w-5 bg-emerald-500 border-2 ${
              resolvedTheme === 'light' ? 'border-white' : 'border-slate-900'
            }`}></span>
          </span>
        </div>

        <div>
          <h1 className={`font-heading font-extrabold text-2xl tracking-tight flex items-center gap-2 justify-center ${
        ...
        <div className={`w-full rounded-2xl p-4 text-left border max-h-[180px] overflow-y-auto ${
          resolvedTheme === 'light'
          ...
          {activeRunningLogs.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {activeRunningLogs.map(l => {
                ...
                return (
                  <div key={l.id} className={`flex items-center justify-between text-xs p-2.5 rounded-xl border ${
                    resolvedTheme === 'light'
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full">
          ...
          <button
            ...
            className={`py-3.5 px-6 rounded-2xl text-sm font-semibold border transition-all ${
              activeRunningLogs.length === 0
